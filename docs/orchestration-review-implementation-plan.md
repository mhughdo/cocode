...
| CHAT-08 | doing | Add SSE `thread.message.delta` / turn events. | Renderer receives incremental updates without refetching the full thread on every event. | Backend chat event payload tests are done; renderer reducer/e2e remains open. | `services/cocoded/internal/httpapi`, `apps/desktop/src/renderer/src` |
...
| CHAT-14 | doing | Replace client-faked SSE previews with persisted message deltas. | UI renders streamed content from persisted/delta events and does not depend on truncated preview reconstruction. | Backend `ChatMessageDelta` events preserve full deltas; renderer consumption/e2e remains open. | `services/cocoded/internal/httpapi`, `services/cocoded/internal/chat`, `apps/desktop/src/renderer/src` |
...
| Chat answer survives only via client-side preview reconstruction. | `CHAT-01`, `CHAT-14` | doing | Chat runs now create/update stable message rows and emit full durable delta events; renderer consumption remains open. |
| Renderer refetches full thread too often. | `CHAT-08`, `CHAT-09` | doing | Backend emits chat message/turn events; UI reducer and refresh removal remain open. |
