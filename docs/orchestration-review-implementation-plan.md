...
| CHAT-15 | doing | Reconcile abandoned running turns on startup/load. | Client aborts or app restarts do not leave `chat_turns` permanently stuck in `running`. | `go test ./internal/chat` covers the service reconciler; app startup/load wiring remains open. | `services/cocoded/internal/chat`, `services/cocoded/internal/app` |
...
| Client abort leaves turns stuck running. | `CHAT-03`, `CHAT-15` | doing | Explicit cancel exists and the service reconciler is implemented; startup/load wiring remains open. |
