...
- `chat/`: centralized review chat, runtime traces, and shared chat composer
  controls.
...
- `findings/`: finding cards, detail/follow-up screens, comments, and finding
  thread rendering.
- `review/`: review session tabs, live data, details, findings board, and
  publish preview.
- `setup/`: new review setup flow and source/diff selection.
- `shell/`: app navigation chrome.
- `shared/`: small reusable UI hooks/components, markdown/code rendering,
  agent output normalization, breadcrumbs, and cross-feature formatting.
