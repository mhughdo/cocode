...
| --- | --- | --- | --- | --- | --- |
| BASE-01 | todo | Add workflow invariant tests for the 8-phase checkpoint order. | The expected phase sequence is encoded in tests and fails if phases are reordered or skipped accidentally. | Orchestrator phase-order unit test. | `services/cocoded/internal/orchestrator/service.go` |
| BASE-02 | todo | Add parser preservation tests for mixed CLI output. | Structured JSON, JSON embedded in text, and raw malformed output are preserved with artifacts and parse status. | Agent output parser tests. | `services/cocoded/internal/agentoutput`, `services/cocoded/internal/orchestrator` |
...
| ID | Status | Task | Acceptance Criteria | Verification | Likely Files |
| --- | --- | --- | --- | --- | --- |
| PROMPT-01 | todo | Move live reviewer prompt to a single embedded markdown source. | Runtime prompt source is not a divergent inline Go string; prompt rendering has a version/hash. | Prompt rendering snapshot/hash test. | `packages/prompts/review-agent.md`, `services/cocoded/internal/orchestrator/service.go` |
| PROMPT-02 | todo | Persist rendered prompt artifacts. | Each agent run records prompt version/hash and enough rendered prompt metadata to trace findings. | Agent run artifact/metadata test. | `services/cocoded/internal/agents`, `services/cocoded/internal/orchestrator`, `services/cocoded/internal/db` |
| PROMPT-03 | todo | Add exact output enums, confidence scale, JSON-only rule, and worked example. | Reviewer prompt lists valid severities/categories and says to output exactly one JSON object. Unknown enum coercion is reduced to fallback behavior only. | Parser tests plus prompt snapshot. | `services/cocoded/internal/agentoutput/candidates.go`, `packages/prompts/review-agent.md` |
| PROMPT-04 | todo | Add severity rubric, stop conditions, and finding cap. | Agents have behavior-mapped severity definitions, `findings: []` guidance, and an explicit maximum finding count. | Prompt snapshot and review fixture evaluation. | `packages/prompts/review-agent.md` |
| PROMPT-05 | todo | Add role/preset overlays to review prompts. | Each configured role contributes role objective, focus areas, boundaries, and `do not flag` guidance to the prompt. | Prompt tests for at least two roles proving different rendered output. | `services/cocoded/internal/orchestrator/service.go`, `services/cocoded/internal/agentpreset`, `docs/presets-and-roles.md` |
| PROMPT-06 | todo | Align reviewer and curator taxonomies. | Curator categories map to the same canonical category enum used by parsed candidates. | Unit tests for category normalization and curation parsing. | `services/cocoded/internal/orchestrator/curation_agent.go`, `services/cocoded/internal/agentoutput/candidates.go` |
| PROMPT-07 | todo | Centralize injection-defense text. | Review, curator, verifier, evidence, and chat prompts share consistent untrusted-context guidance. | Prompt snapshot tests across prompt builders. | `services/cocoded/internal/orchestrator`, `services/cocoded/internal/chat`, `packages/prompts` |
| PROMPT-08 | todo | Wire and test `PromptTemplate` override behavior. | Runtime prompt override has a deliberate API, is tested, and is included in prompt hash/provenance. | Service construction/rendering tests. | `services/cocoded/internal/httpapi/router.go`, `services/cocoded/internal/orchestrator/service.go` |
| PROMPT-09 | todo | Add prompt output-contract schema tests. | Prompt examples and parser schema stay in sync; schema drift fails tests. | Golden tests comparing prompt enum text and parser known values. | `packages/prompts`, `services/cocoded/internal/agentoutput` |
| PROMPT-10 | todo | Add review-relevant project-rules filtering guidance. | Agents are told how to use project rules without treating setup/style-only text as review truth. | Prompt snapshot test with AGENTS/CLAUDE fixture. | `packages/prompts/review-agent.md`, `services/cocoded/internal/contextbundle` |
...
| ID | Status | Task | Acceptance Criteria | Verification | Likely Files |
| --- | --- | --- | --- | --- | --- |
| VERIFY-01 | todo | Gate curator-asserted verification status. | Curator may propose `verified`, but persisted status cannot exceed `locally_supported` unless deterministic anchor/content checks pass. | Hallucinated-location fixture cannot become verified. | `services/cocoded/internal/orchestrator/service.go`, `services/cocoded/internal/evidence/service.go` |
| VERIFY-02 | todo | Require deterministic anchor validation for curated location overrides. | Curated primary location must point to changed code or be downgraded/rejected with provenance. | Curation fixture with unchanged/nonexistent line is downgraded. | `services/cocoded/internal/orchestrator/service.go` |
| VERIFY-03 | todo | Add quoted-code matching for finding and evidence locations. | Evidence status accounts for whether quoted/observed code still matches file content. | Evidence verifier tests with matching and stale snippets. | `services/cocoded/internal/evidence`, `services/cocoded/internal/orchestrator/verifier_agent.go` |
| VERIFY-04 | todo | Make deterministic counter-evidence classification reachable. | Local verifier can classify concrete contradictions as counter-evidence and mark likely false positives when appropriate. | Test reaches `likely_false_positive` without an LLM verifier. | `services/cocoded/internal/evidence/service.go` |
| VERIFY-05 | todo | Add consensus confidence across distinct source agents. | Merged findings combine confidence from independent agents using a documented formula, not only a tiebreaker. | Dedupe/curation tests for one-agent vs multi-agent claims. | `services/cocoded/internal/findingengine`, `services/cocoded/internal/orchestrator` |
| VERIFY-06 | todo | Preserve verifier disagreement explicitly. | Conflicting verifier outputs are stored/displayed instead of last-writer-wins. | Verifier fixture shows support and refutation provenance. | `services/cocoded/internal/orchestrator/verifier_agent.go`, `services/cocoded/internal/evidence` |
...
| H5: Curator can mint verified status without deterministic backstop. | `VERIFY-01`, `VERIFY-02` | todo | Also interacts with prompt injection boundaries. |
...
| Verified means file/line exists, not code content match. | `VERIFY-03` | todo | Adds quoted-code/content matching. |
| Deterministic counter-evidence path is unreachable/weak. | `VERIFY-04` | todo | Makes likely-false-positive local path reachable. |
| No consensus signal across agents. | `VERIFY-05` | todo | Adds distinct-agent confidence reconciliation. |
...
| Verifier disagreement is last-writer-wins. | `VERIFY-06` | todo | Store/display disagreement provenance. |
