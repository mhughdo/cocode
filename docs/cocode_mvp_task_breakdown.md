...
T240-T267 track functional UI coverage. Visual quality and Codex Desktop-style polish are tracked explicitly here as separate rows so broad UI completion cannot hide design-system gaps.
...
| T240A | Codex-style visual polish pass | Align shell, source setup, configure, composer, and shared tokens with the Codex Desktop App design system. | Done | T240-T246 | T263A | Live Computer check verifies no fake empty-state review pane on first run, calmer sidebar/top nav proportions, consistent Geist type scale, lower desktop minimum width, and Codex-like panel/button treatment. |
...
| T251 | Implement chat composer | Review/follow-up composer with runtime/model/reasoning/tool/permission controls. | Done | T249, T092 | T280 | Follow-up composer submits finding-scoped questions through existing endpoints, maps runtime/permission controls into context policy, supports agent/model selection, and keeps review-wide submit disabled until a review-level backend endpoint exists. |
...
| T254 | Implement review controls UI | Pause, cancel review, cancel one agent. | Done | T150, T151, T100 | T250 | Review controls support pause/resume/cancel, summaries include concrete agent-run rows, and running agent cards can cancel a single queued/running agent without canceling the whole session. |
...
| T263A | Evidence Map visual polish pass | Make Evidence Map usable as a product inspection surface, not just a graph placeholder. | Done | T263 | T264 | Seeded Evidence Map has a responsive hierarchy/graph/context layout, graph grid canvas, clearer node/edge styling, compact call-path rail, readable selected context, and targeted E2E coverage still passes. |
...
| T327 | Implement secret redaction UI | Show redaction status/report in configure/review detail. | Done | T130, T247 | T333 | Review details can preview redaction counts, detector summaries, affected items, and persisted report artifacts. |
| T328 | Implement credentials settings | Add GitHub token and optional CLI credential refs/settings. | Done | T037, T073 | T248 | GitHub token settings use Electron safe storage; cocoded stores only credential refs/metadata, PR ingestion reads the saved token through desktop IPC, and CLI configs can carry credential refs without raw secrets. |
...
| T366 | Create frontend component tests | Tests for finding card, evidence card, graph node, copy buttons. | Done | T242, T257, T261 | T367 | Renderer tests cover shared loading/error/empty states plus finding cards, evidence cards, Evidence Map graph nodes/edges, and copy-path/copy-finding actions. |
| T367 | Create E2E test harness | Launch Electron with fake backend or seeded DB. | Done | T014, T040 | T368 | Playwright launches the built Electron app with isolated cocoded data/artifact dirs, verifies the preload backend bridge, and authenticates against the spawned backend via `@cocode/desktop e2e`. |
| T368 | E2E: New thread to configure | Test PR URL/local branch flow to Configure Review. | Done | T245, T246, T367 | T369 | Playwright opens a temp git repo through the Electron repository bridge, creates a branch-compare snapshot, and verifies Configure Review loads the expected changed file. |
| T369 | E2E: Run fake review | Start review with fake agents and see findings. | Done | T142, T250, T367 | T370 | Playwright registers an executable fake JSON reviewer, starts a branch-compare review through the UI, and verifies the live Findings board renders the normalized verified finding. |
| T370 | E2E: Finding detail and Evidence Map | Open finding detail and Evidence Map. | Done | T260, T263, T367 | T371 | Playwright seeds demo data, opens the accepted PR #42 finding detail, verifies evidence content, and opens Evidence Map with hierarchy, SVG graph, call path, and selected context rendered. |
| T371 | E2E: Copy packet | Accept findings and copy selected packet. | Done | T291, T292, T367 | T372 | Playwright opens seeded accepted findings, copies the selected markdown packet through the real Electron clipboard bridge, and verifies clipboard content. |
| T372 | E2E: GitHub preview | Preview publish payload with fake GitHub. | Done | T296, T367 | T373 | Playwright builds the seeded GitHub preview and verifies review body, inline comments, checklist, and anchored file path render. |
| T373 | Reliability test: agent timeout | Slow fake agent times out without losing other findings. | Done | T099, T149, T361 | T374 | Orchestrator test runs one successful reviewer and one timed-out reviewer, verifies the session completes with partial-failure telemetry, preserves the successful finding/candidate, and exposes succeeded/timed_out run counts. |
| T374 | Reliability test: app restart | Restart app/backend and reload previous session. | Done | T143, T031, T361 | T375 | Playwright relaunches Electron against the same seeded data dir and verifies the previous review, accepted finding, evidence item, and Publish selection persist across backend/app restart. |
| T375 | Evaluation harness v1 | Run reviews on golden repos and measure expected findings. | Done | T363-T365, T142 | T376 | `pnpm backend:eval` runs deterministic golden-repo checks and emits JSON metrics for accepted expected findings, missing expected findings, false positives, precision-ish score, duration, and unavailable cost metadata. |
| T376 | Add dogfood checklist | Manual test checklist for real PRs. | Done | Core MVP | T390 | `docs/dogfood_checklist.md` covers setup, snapshot, run, triage, Evidence Map, follow-up, copy, publish, required PR mix, and result recording; the MVP release checklist links to it. |
