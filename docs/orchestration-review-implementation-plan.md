...
| CHAT-07 | done | Add thread summaries and compaction. | Long chats include compact summaries and recent turns instead of unbounded raw history. | `go test ./internal/chat` from `services/cocoded`. | `services/cocoded/internal/chat` |
| CHAT-08 | done | Add SSE `thread.message.delta` / turn events. | Renderer receives incremental updates without refetching the full thread on every event. | `go test ./internal/chat`; focused HTTP chat tests; `pnpm --filter @cocode/desktop test -- chat-thread-events centralized-chat-screen`; `pnpm --filter @cocode/desktop typecheck`. | `services/cocoded/internal/httpapi`, `apps/desktop/src/renderer/src` |
| CHAT-09 | done | Debounce or remove full-thread refresh loops. | Chat UI no longer does high-frequency full refetches during streaming. | `rg` confirms the event-length refresh and active polling loop are removed; frontend chat tests pass. | `apps/desktop/src/renderer/src` |
| CHAT-10 | done | Unify finding follow-ups into centralized chat. | Finding-scoped questions are central chat turns mirrored into the legacy finding-thread response, with finding/evidence-map context refs. | `go test ./internal/followup ./internal/chat ./internal/httpapi -run 'TestFindingQuestionEndpointRunsAgentAndPersistsMessages|TestEvidenceMapQuestionEndpointRunsVerifierWithGraphContext|TestCreateTurnPersistsContextRefs|TestChatTurnStateMachine|TestReconcileInterruptedTurnsMarksNonTerminalTurns|TestFollowup'` from `services/cocoded`. | `services/cocoded/internal/followup`, `services/cocoded/internal/chat`, `services/cocoded/internal/httpapi`, `apps/desktop/src/renderer/src/lib/api.ts` |
| CHAT-11 | done | Include prior messages in finding-scoped context. | Finding follow-ups run through central chat, so prompt history uses the existing thread summary/recent-message path instead of a cold standalone prompt. | Same focused follow-up/chat/httpapi test run as `CHAT-10`. | `services/cocoded/internal/followup/question.go`, `services/cocoded/internal/chat` |
...
| CHAT-13 | done | Make thread read APIs side-effect-free and efficient. | Loading a thread does not perform server-side read-modify-write cleanup or N+1 sync work. | `go test ./internal/chat`; focused HTTP chat tests from `services/cocoded`. | `services/cocoded/internal/chat/service.go`, `services/cocoded/internal/httpapi/chat.go` |
| CHAT-14 | done | Replace client-faked SSE previews with persisted message deltas. | UI renders streamed content from persisted/delta events and does not depend on truncated preview reconstruction. | Backend `ChatMessageDelta` events preserve full deltas; renderer reducer tests cover stale create, delta, and final-update ordering. | `services/cocoded/internal/httpapi`, `services/cocoded/internal/chat`, `apps/desktop/src/renderer/src` |
| CHAT-15 | done | Reconcile abandoned running turns on startup/load. | Client aborts or app restarts do not leave `chat_turns` permanently stuck in `running`; cocoded startup reconciles non-terminal turns. | `go test ./internal/chat ./internal/httpapi -run 'TestReconcileInterruptedTurnsMarksNonTerminalTurns|TestReviewSessionChatThreadEndpointSeedsAndAnswers|TestFindingQuestionEndpointRunsAgentAndPersistsMessages|TestEvidenceMapQuestionEndpointRunsVerifierWithGraphContext'` from `services/cocoded`. | `services/cocoded/internal/chat`, `services/cocoded/internal/httpapi` |
...
| Renderer refetches full thread too often. | `CHAT-08`, `CHAT-09` | done | Centralized chat applies chat SSE events locally and removed event-length/poll refresh loops. |
