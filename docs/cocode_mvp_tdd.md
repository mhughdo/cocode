...
Security smoke tests live in `internal/securitysmoke` and are called explicitly from CI. The suite exercises the local auth middleware, Origin preflight/allow/deny behavior, path sandbox traversal and symlink escapes, env allowlist normalization/resolution, and secret redaction leak checks.
